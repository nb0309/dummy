...
applies_when:
  ...
  ax_role: [main, heading, text, region]
axe_ids: [heading-order, empty-heading, page-has-heading-one]
signals:
  - field: parent_html
    look_for: "heading levels that skip downwards (e.g. h1 then h3) or are out of logical order"
  - field: element_html_raw
    look_for: "visually-styled 'heading' text using <b>/<strong>/<p>/font-size instead of <h1>-<h6>"
...
- **Skipped levels**: heading levels jump downward out of order (e.g. `<h1>` →
  `<h3>`, or `<h2>` → `<h5>`). Check `parent_html`/`sr_reading_order` for the full
...
- **Text formatting used as a heading**: text that is visually a heading but
  ...
  `<h1>`–`<h6>`, so no `heading` role is exposed in `sr_reading_order`/`ax_subtree`.
- An **empty heading** element conveying no text.

## Pass criteria
- Headings use `<h1>`–`<h6>` in a logical, non-skipping order; `sr_reading_order`
